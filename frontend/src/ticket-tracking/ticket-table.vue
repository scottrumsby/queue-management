      ...
      <b-col>
        <b-table :fields="fields"
          ...
          <template slot="status" slot-scope="row">
            <b-badge variant="success" v-if="row.item.status === 'active' ">active</b-badge>
            <b-badge variant="secondary" v-if="row.item.status === 'done' ">done</b-badge>
          </template>
...
export default {
  ...
  data() {
    return {
      ...
      'invited_service_reqs': [
        {
          'service.parent.service_name':'Exams-ITA',
          'service.service_name': 'Exam-Capture details',
          'status': 'active',
          quantity: 1,
          editBut: 'edit'
        },
        {
          'service.parent.service_name':'Exams-NonITA',
          'service.service_name': 'Exam-Booking citizen ',
          'status': 'done',
          quantity: 2,
          editBut: 'edit'
        },
        {
          'service.parent.service_name':'Office-Admin',
          'service.service_name': 'Admin-Inventory',
          'status': 'done',
          quantity: 1,
          editBut: 'edit'
        },
  ...
  computed: {
    ...mapState(['ticketModal','ticketModalForm']),
